//TexasHoldEm.cpp: This is the source file where the TexasHoldEm class and relevant functions are defined
...
//Indexes of face down cards 0, 1, 
//Indexes of face up cards 2, 3, 4, 5, 6
	...
	: dealer(0) //dealer is started at position 0
{
	...
	vector<bool> v = { 0, 0, 1, 1, 1, 1, 1 };
	face = v;
}

int TexasHoldEm::before_turn(Player & currPlayer)
{
...
int TexasHoldEm::flop(vector<shared_ptr<Player>> players)
{
	try 
	{
		Card firstCard = d.remove_card();
		Card secondCard = d.remove_card();
		Card thirdCard = d.remove_card();

		for (shared_ptr<Player> player : players) {
			playerToCards[player->name].push_back(firstCard);
			playerToCards[player->name].push_back(secondCard);
			playerToCards[player->name].push_back(thirdCard);
		}
...
int TexasHoldEm::turn_river(vector<shared_ptr<Player>> players)
{
	Card nextCard = d.remove_card();
	for (shared_ptr<Player> player : players) {
		playerToCards[player->name].push_back(nextCard);
	}
	return success;
}

int TexasHoldEm::turn(Player & currPlayer)
{
	try
	{
		string name = currPlayer.name;
...
void TexasHoldEm::setHand(Player & currPlayer)
{
	set<vector<Card>> b;
...
void TexasHoldEm::autoLeaveGame(Player &currPlayer)
{
...
void TexasHoldEm::print_hand(Player p)
{
	string Pname = p.name;
	cout << Pname << " Hand: ";
